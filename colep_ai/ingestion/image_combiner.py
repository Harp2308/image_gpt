...
# def _sift_features(gray_img, sift):
#     return sift.detectAndCompute(gray_img, None)


# def locate_crop(kp_page, des_page, crop, sift, crop_path=""):
#     gray_crop = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
#     kp_crop, des_crop = sift.detectAndCompute(gray_crop, None)

#     if des_page is None or des_crop is None:
#         logger.warning(f"{crop_path}: no SIFT descriptors (blank/low-detail image)")
#         return None
...
#     if len(good) < 10:
#         logger.warning(f"{crop_path}: only {len(good)} good matches (<10) — rejecting")
#         return None
...
#     if H is None:
#         logger.warning(f"{crop_path}: homography failed despite {len(good)} good matches")
#         return None
...
# def reconstruct(page, kp_page, des_page, sift, crop_paths):
#     """page: already-loaded image. kp_page/des_page: computed ONCE per page, reused across steps."""
#     placements = []
#     min_x, min_y, max_x, max_y = 1e9, 1e9, -1e9, -1e9

#     for path in crop_paths:
#         crop = cv2.imread(path)
#         result = locate_crop(kp_page, des_page, crop, sift, crop_path=path)
#         if result is None:
#             logger.error(f"Could not locate: {path} — excluded from reconstruction")
#             continue
...
#         h, w = crop.shape[:2]
#         xx, yy = int(x - min_x), int(y - min_y)
#         canvas[yy:yy + h, xx:xx + w] = crop
...
#     kp_page, des_page = sift.detectAndCompute(cv2.cvtColor(page, cv2.COLOR_BGR2GRAY), None)
...
#         result = reconstruct(page, kp_page, des_page, sift, [str(p) for p in crop_paths])
#         if result is None:
#             logger.error(f"Step {step_key}: reconstruct returned None — skipping")
#             status[step_key] = False
#             continue
